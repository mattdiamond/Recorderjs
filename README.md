...
- **sourceGain**              - (*optional*) Sets the gain of the microphone input. Gain is an a-weighted value between `0` and `1`. Defaults to `1`
...
```js
rec.setSourceGain( gain )
```

**setSourceGain** will set the volume on what will be passed to the recorder. Gain is an a-weighted value between `0` and `1`.
